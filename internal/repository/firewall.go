...
const (
	...
	getDomainListInfoQuery = `
		SELECT dl.id, dl.name, (SELECT COUNT(*) FROM domain_list_domains dld WHERE dld.domain_list_id = dl.id) AS domain_count
	...
	listDomainListsQuery = `
		SELECT dl.id, dl.name, COUNT(dld.domain) AS domain_count
		FROM domain_lists dl
...
func (p *PostgresFirewallRepository) CreateDomainList(
	...
	row := p.db.QueryRow(ctx, createDomainListQuery, list.Name)
	...
	return &model.DomainListInfo{ID: id, Name: name, DomainCount: len(list.Domains)}, nil
}
...
func (p *PostgresFirewallRepository) GetDomainList(ctx context.Context, id uuid.UUID) (*model.DomainList, error) {
	...
	var dbID uuid.UUID
	var name string
	if err := row.Scan(&dbID, &name); err != nil {
		return nil, handleError(err, "failed to scan domain list: %w", err)
	}

	rows, err := p.db.Query(ctx, getDomainListDomainsQuery, dbID)
	if err != nil {
	...
	return &model.DomainList{ID: dbID, Name: name, Domains: domains}, nil
}
...
func (p *PostgresFirewallRepository) GetDomainListInfo(
	...
	if err := row.Scan(&info.ID, &info.Name, &info.DomainCount); err != nil {
...
func (p *PostgresFirewallRepository) ListDomainLists(ctx context.Context) ([]model.DomainListInfo, error) {
	...
	for rows.Next() {
		...
		var name string
		var domainCount int
		if err = rows.Scan(&id, &name, &domainCount); err != nil {
			return nil, handleError(err, "failed to scan domain list: %w", err)
		}
		lists = append(lists, model.DomainListInfo{ID: id, Name: name, DomainCount: domainCount})
	}
...
func (p *PostgresFirewallRepository) GetFirewallRule(ctx context.Context, id uuid.UUID) (*model.FirewallRule, error) {
	...
	if err := row.Scan(&rule.ID, &rule.Name, &rule.DomainListID, &rule.Action, &rule.BlockResponseType, &rule.BlockResponse, &rule.Priority); err != nil {
